...
const desktopNavItems = [
  ...
  { href: "/penumpang", label: "Penumpang" },
  {
    ...
    label: "Tentang",
    items: [
      { href: "/tentang", label: "Profil Bandara" },
...
const mobileNavItems: { href: string; label: string; icon: string }[] = [];
...
const serviceSidebarItems = [
  {
    href: "/layanan-informasi#daftar-informasi",
...
const aboutRoutes = new Set(["/tentang", "/fasilitas", "/regulasi", "/ppid", "/unit-kerja"]);
...
export default function FlightLayout({
  ...
  const sidebarItems = isAboutSidebar
    ...
    : isExploreSidebar
      ? exploreSidebarItems
      : isInformationSidebar
        ? informationSidebarItems
        : isServiceSidebar
          ? serviceSidebarItems
          : mobileNavItems;

  const isItemActive = (href: string) => {
    if (href.startsWith("/layanan-informasi#")) {
      const [basePath, hashId] = href.split("#");
  ...
  const isDesktopItemActive = (item: (typeof desktopNavItems)[number]) =>
    isNavGroupActive(item.href) || Boolean(item.items?.some((subItem) => isItemActive(subItem.href)));
  ...
  useEffect(() => {
    if (pathname !== "/layanan-informasi") return;
    ...
    const setSectionFromScroll = () => {
      ...
      const currentSection = sections.reduce((activeSection, section) => {
        ...
        const activeTop = activeSection.getBoundingClientRect().top;
        const headerOffset = 200; // Offset increased slightly to ensure smooth triggering

        if (sectionTop <= headerOffset && sectionTop > activeTop) {
          return section;
        }
    ...
    // Run initially and after a short delay for DOM hydration
    setSectionFromScroll();
    const timeoutId = setTimeout(setSectionFromScroll, 300);
  ...
  return (
    <>
      ...
      <div className="min-h-screen overflow-x-hidden bg-[#111928] text-white font-[Poppins,sans-serif] leading-relaxed">
        <header className="fixed left-0 right-0 top-0 z-[100] border-b border-white/10 bg-[#111928]/80 backdrop-blur-md">
                    ...
                    >
                      {item.items.map((subItem) => (
                        <Link
                          key={subItem.href}
                          href={subItem.href}
                          className={`block px-4 py-2 text-sm transition hover:bg-gray-700 hover:text-[#facc15] ${isItemActive(subItem.href) ? "bg-gray-700 text-[#facc15]" : "text-gray-300"
                            }`}
                        >
                          {subItem.label}
                        </Link>
            ...
            <nav className={`flex-1 min-h-0 overflow-y-auto pr-2 flex flex-col ${isSectionSidebar ? "space-y-0" : "space-y-6"}`}>
              {sidebarItems.map((item) => (
                <Link
                  ...
                  href={item.href}
                  onClick={() => setIsMenuOpen(false)}
                  className={`flex items-center gap-4 transition ${isSectionSidebar
                    ? `border-b border-white/5 py-4 text-[18px] font-medium ${isItemActive(item.href) ? "text-[#facc15]" : "text-gray-200 hover:text-[#facc15]"
                    }`
                  ...
                  style={{
                    fontFamily: isSectionSidebar ? "'Poppins', sans-serif" : "'Playfair Display', serif",
                  }}
                >
                  <i className={`fa-solid ${item.icon} ${isSectionSidebar ? "w-6 text-base" : "text-lg"}`}></i>
                  {item.label}
                </Link>
              ))}
            </nav>
        ...
        <footer className="border-t border-gray-800 bg-black/40 pb-28 pt-20 md:pb-16">
          <div className="mx-auto max-w-7xl px-6">
                ...
                <div className="mx-auto flex w-fit flex-col items-center">
                  ...
                  <ul className="space-y-4 self-start text-left">
                    ...
                    <li className="group flex items-center gap-4">
                      ...
                      <a
                        href="https://www.google.com/maps/place/Bandar+Udara+Tardamu/@-10.4910406,121.845765,1083m/data=!3m1!1e3!4m6!3m5!1s0x2c51efa802c41119:0xa4ad04e0aea134e6!8m2!3d-10.4941185!4d121.8454013!16s%2Fm%2F0pcftry?entry=ttu&g_ep=EgoyMDI2MDUxMy4wIKXMDSoASAFQAw%3D%3D"
                        target="_blank"
                        rel="noreferrer"
                ...
                <div className="flex justify-center gap-4">
                  {socialLinks.map((item) => (
                    <a
                      ...
                      className="flex h-10 w-10 items-center justify-center rounded-full border border-white/5 bg-[#1f2937] text-gray-400 transition-all duration-300 hover:-translate-y-1 hover:bg-[#facc15] hover:text-[#111928]"
                    >
            ...
            <div className="flex flex-col items-center justify-center gap-4 border-t border-white/5 pt-8 text-[10px] uppercase tracking-[0.2em] text-gray-600 md:flex-row">
        ...
        <nav className="fixed bottom-0 left-0 right-0 z-[130] flex h-16 items-center justify-around bg-[#111928]/90 px-2 backdrop-blur-md lg:hidden">
          ...
          <Link
            href="/informasi"
            className={`flex flex-col items-center gap-1 ${isInformationActive ? "text-[#facc15]" : "text-gray-500"
              }`}
          >
