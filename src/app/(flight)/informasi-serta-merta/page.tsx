...
function getPageHref(page: number) {
  return page <= 1 ? "/informasi-serta-merta" : `/informasi-serta-merta?page=${page}`;
}
...
export async function generateMetadata({ searchParams }: PageProps): Promise<Metadata> {
  ...
  return buildPageMetadata({
    ...
    path: page > 1 ? `/informasi-serta-merta?page=${page}` : "/informasi-serta-merta",
  });
}
...
export default async function InformasiSertaMertaPage({ searchParams }: PageProps) {
  ...
  try {
    ...
    console.error("Failed to render /informasi-serta-merta:", error);
    loadFailed = true;
  }
